...
WORK_DIR = "/GeC/gecsamba/gecscraping/live_scripts/"
LOG_DIR = "/GeC/gecftp/logs/"
LOCK_FILE = "/GeC/gecsamba/gecscraping/live_scripts/dgscraping.lock"
...
def runSource(source):
    ...
    runStr = "cd {} && python3 -u {}.py >> {}/{}-$(date +\%d-\%m-\%Y).log 2>&1 &".format(WORK_DIR, scriptName, LOG_DIR, scriptName)
    os.system(runStr)
...
sql = "select internal_code, TO_CHAR(scheduled_time, 'HH24:MI') as scheduled_time from app_sources where status in (1,13,15,16,17) and instance_name = 2 and scheduled_time is not null"
